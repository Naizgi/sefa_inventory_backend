...
from app.models import (
    ...
    TempItem, TempItemStatus, SaleItem, User
)
...
def get_dashboard(
    ...
    for stock in stock_query.all():
        product = db.query(Product).filter(Product.id == stock.product_id).first()
        if product:
            inventory_value += float(stock.quantity) * float(product.cost)
    ...
    pending_purchase_value = sum(float(order.total_amount) for order in pending_purchases)
    ...
    for product in top_products:
        top_products_data.append({
            ...
            "name": product.name,
            "sku": product.sku,
            "total_sold": int(product.total_sold) if product.total_sold else 0,
    ...
    if is_privileged:
        ...
        for loan in pending_loans:
            pending_approvals.append({
                ...
                "description": f"Customer: {loan.customer_name} | Amount: {float(loan.total_amount):,.2f}",
                "requested_by": db.query(User).filter(User.id == loan.created_by).first().name if loan.created_by else "Unknown",
        ...
        for purchase in pending_purchase_approvals:
            pending_approvals.append({
                ...
                "description": f"Supplier: {purchase.supplier} | Amount: {float(purchase.total_amount):,.2f}",
                "requested_by": db.query(User).filter(User.id == purchase.created_by).first().name if purchase.created_by else "Unknown",
        ...
        for refund in pending_refunds:
            sale = db.query(Sale).filter(Sale.id == refund.original_sale_id).first()
            pending_approvals.append({
                "id": refund.id,
                "type": "refund",
                "title": f"Refund #{refund.refund_number}",
                "description": f"Amount: {float(refund.refund_amount):,.2f} | Reason: {refund.refund_reason[:50]}",
                "requested_by": db.query(User).filter(User.id == refund.user_id).first().name if refund.user_id else "Unknown",
        ...
        for damaged in pending_damaged:
            ...
            pending_approvals.append({
                ...
                "description": f"Product: {product.name if product else 'Unknown'} | Quantity: {float(damaged.quantity)}",
                "requested_by": db.query(User).filter(User.id == damaged.reported_by).first().name if damaged.reported_by else "Unknown",
                "created_at": damaged.reported_at,
                "status": "pending"
            })
        ...
        if is_admin:
            ...
            for temp_item in pending_temp_items:
                pending_approvals.append({
                    ...
                    "requested_by": db.query(User).filter(User.id == temp_item.registered_by).first().name if temp_item.registered_by else "Unknown",
                    "created_at": temp_item.registered_at,
                    "status": "pending"
                })
    
    # Sort pending approvals by created_at (newest first)
    pending_approvals.sort(key=lambda x: x.get('created_at', datetime.min), reverse=True)
    ...
    # ==================== BANK PURCHASE SUMMARY (For Admin/Privileged) ====================
    ...
    vat_summary_data = {
        "total_vat_paid": 0,
        "average_vat_rate": 0,
        "vat_purchases_count": 0,
        "non_vat_purchases_count": 0
    }
    ...
    for order in vat_orders:
        if order.vat_amount and order.vat_amount > 0:
            total_vat_paid += float(order.vat_amount)
            vat_rate_sum += float(order.vat_rate) if order.vat_rate else 0
            vat_purchases += 1
        else:
            non_vat_purchases += 1
    ...
    for sale in recent_sales:
        recent_activities.append({
            "id": f"sale_{sale.id}",
            "type": "sale",
            "message": f"Sale #{sale.invoice_number} - {float(sale.total_amount):,.2f}",
